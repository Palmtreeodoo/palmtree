...
class SmsNotification(models.Model):
    ...
    group_type = fields.Selection([('group', 'Group'),
                                   ...
                                   ('individual', 'Individual Member/Number')], string="Send SMS To", default='group',
                                  help="This field is used to send the message for a single customer or group of customer.")
    ...
    #         group) for groups in self.group_ids for group in groups if self.group_ids]
    ...
    #     for i in self:
    #         # i.to=""
    #         i.to = self.get_mobile_number([self._get_partner_mobile(partner)
    #                                        for partners in self.partner_ids for partner in partners if partner.mobile])

    @api.onchange('partner_ids')
    def add_partner_member_number(self):
        self.to = self.get_mobile_number([self._get_partner_mobile(partner) for partner in self.partner_ids if partner.mobile or partner.phone])
    ...
    def onchange_sms_gateway_config_id(self):
        for i in self:
            if i.group_type == "multiple":
                i.add_partner_member_number()
            elif i.group_type == "group":
                i.add_group_member_number()
            elif i.group_type == "individual":
                i.add_partner_number()
